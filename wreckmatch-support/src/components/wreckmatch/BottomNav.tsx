...
const navItems = [
  { href: WM.home, key: "home" as const, icon: Home },
  { href: WM.community, key: "community" as const, icon: Users },
  { href: WM.matches, key: "matches" as const, icon: Scale },
  { href: WM.help, key: "help" as const, icon: HeartHandshake, highlight: true },
  { href: WM.profile, key: "profile" as const, icon: User },
] as const;
    ...
    >
      <div className="mx-auto flex max-w-lg items-stretch justify-between gap-0.5 px-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1.5">
        {navItems.map(({ href, key, icon: Icon, ...rest }) => {
          ...
          const active =
            pathname === href || (href !== WM.home && pathname.startsWith(href));
